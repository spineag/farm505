...
//        return _SHADOW;
    ...
    public static function get NEW_SHADOW():DropShadowFilter {
        return null; //new DropShadowFilter(2, .8, 0, 1, 1.0, 0.5);
    }

    public static function get SHADOW_LIGHT():DropShadowFilter {
        if (!_SHADOW_LIGHT) _SHADOW_LIGHT = null; //new DropShadowFilter(2, .8, 0, .7, .5, 0.5);
        return _SHADOW_LIGHT;
    }

    public static function get SHADOW_TINY():DropShadowFilter {
        if (!_SHADOW_TINY) _SHADOW_TINY = null; //new DropShadowFilter(1, 0.8, 0, .5, .5, 0.5);
        return _SHADOW_TINY;
    }

    public static function get SHADOW_TOP():DropShadowFilter {
        if (!_SHADOW_TOP) _SHADOW_TOP = null; //new DropShadowFilter(1, -.8, 0, 1, 1.0, 0.5);
        return _SHADOW_TOP;
    }

    public static function get RED_STROKE():GlowFilter {
        if (!_RED_STROKE) _RED_STROKE = null; //new GlowFilter(Color.RED, 4, 2, 1);
        return _RED_STROKE;
    }

    public static function get GREEN_STROKE():GlowFilter {
        if (!_GREEN_STROKE) _GREEN_STROKE = null; //new GlowFilter(Color.GREEN, 4, 2, 1);
        return _GREEN_STROKE;
    }

    public static function get YELLOW_STROKE():GlowFilter {
        if (!_YELLOW_STROKE) _YELLOW_STROKE = null; //new GlowFilter(Color.YELLOW, 4, 2, 1);
        return _YELLOW_STROKE;
    }

    public static function get WHITE_STROKE():GlowFilter {
        if (!_WHITE_STROKE) _WHITE_STROKE = null; //new GlowFilter(Color.WHITE, 4, 2, 1);
        return _WHITE_STROKE;
    }

    public static function get BUILD_STROKE():GlowFilter {
        if (!_BUILD_STROKE) _BUILD_STROKE = null; //new GlowFilter(0xeffd98, 4, 2, 1);
        return _BUILD_STROKE;
    }

    public static function getButtonClickFilter():ColorMatrixFilter {
        var f:ColorMatrixFilter = null; //new ColorMatrixFilter();
