...
public class MarketHint {
    ...
    public function showIt(_dataId:int, sX:int, sY:int, source:Sprite,noResource:Boolean = false): void {
        ...
        if (g.dataResource.objectResources[_dataId].buildType == BuildType.PLANT) {
            ...
            _txtCount.x = 30;
            _txtCount.y = 70;
            ManagerFilters.setStrokeStyle(_txtCount, ManagerFilters.TEXT_LIGHT_BLUE_COLOR);
            _txtSklad = new TextField(70,20,'На складе:');
            _txtSklad.format.setTo(g.allData.bFonts['BloggerBold14'],12,ManagerFilters.TEXT_LIGHT_BLUE_COLOR);
            _txtSklad.x = -55;
            _txtSklad.y = 75;
            ManagerFilters.setEmptyStyle(_txtSklad);
            _source.x = start.x + source.width/2;
