...
public class LockedLandItem {
    ...
    public function fillWithCurrency(count:int):void {
        var icon:Image = new Image(g.allData.atlas['interfaceAtlas'].getTexture('coin'));
        MCScaler.scale(icon, 70, 70);
        icon.x = 10;
        icon.y = 10;
        source.addChild(icon);
        if (g.user.softCurrencyCount >= count) {
            var gal:Image = new Image(g.allData.atlas['interfaceAtlas'].getTexture('galo4ka'));
    ...
    public function fillWithResource(id:int, count:int):void {
        ...
        if (g.userInventory.getCountResourceById(id) >= count) {
            ...
            var im:Image = new Image(g.allData.atlas['interfaceAtlas'].getTexture("btn2"));
            im.width = im.width/2;
            txt.x = 310;
            txt.y = 15;
            source.addChild(txt);
