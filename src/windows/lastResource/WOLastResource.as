...
public class WOLastResource extends WindowMain {
    ...
    public function WOLastResource() {
        ...
        var txt:TextField;
        txt = new TextField(150,50,"ВНИМАНИЕ!");
        txt.format.setTo(g.allData.bFonts['BloggerBold24'],22,Color.WHITE);
        txt.filter = ManagerFilters.TEXT_STROKE_BLUE;
        txt.x = -75;
        txt.y = -130;
        _source.addChild(txt);
        txt = new TextField(420,60,"Вы подтверждаете использование этого ресурса? После этого у вас не останется семян для посадки!");
