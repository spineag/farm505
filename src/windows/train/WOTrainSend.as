...
public class WOTrainSend extends WindowMain {
    ...
    public function WOTrainSend() {
        ...
        var txt:TextField;
        txt = new TextField(300,200,"ПОГРУЗКА НЕ ЗАВЕРШЕНА!",g.allData.bFonts['BloggerBold24'],22,Color.WHITE);
        txt.nativeFilters = ManagerFilters.TEXT_STROKE_BLUE;
        txt.x = -150;
        txt.y = -210;
        _source.addChild(txt);
        txt = new TextField(350,50,"Отправить корзину не загрузив полностью?",g.allData.bFonts['BloggerMedium18'],18,Color.WHITE);
        txt.nativeFilters = ManagerFilters.TEXT_STROKE_BLUE;
        txt.x = -180;
        txt.y = -110;
        _source.addChild(txt);
